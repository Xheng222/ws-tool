...
use std::{collections::HashSet, fs, process::Stdio};

use crossterm::style::Stylize;

use crate::{
    commands::{
        models::{ProjectStatus, SVNLogType}, utils::{callback_for_log_xml, check_project_exists, check_url_exists, validate_folder_name}, utils_branch::get_project_branches, utils_clean_workspace::ensure_clean_workspace
    },
    core::{
        app::App, context::check_and_repair_workspace, error::{AppError, AppResult}, svn::{svn_checkout, svn_cleanup, svn_cleanup_workspace, svn_copy, svn_delete, svn_list, svn_mkdir, svn_propset, svn_switch, svn_update}, svn_repo::{svnadmin_create, svnadmin_dump, svnadmin_load, svndumpfilter}
...
pub fn handle_list(app: &App, list_all: bool) -> AppResult<()> {
    ...
    for line in active_string.lines() {
        ...
        if !name.is_empty() && name != ".gitignore" {
...
pub fn handle_new(app: &App, project_name: &str) -> AppResult<()> {
    ...
    svn_mkdir(&["--parents", &trunk_url, &branches_url, &tags_url, "-m", &format!("Init {}", project_name)])?;
    ...
    if app.svn_ctx.get_current_project_name().is_empty() {
        // 当前不在任何项目中，使用 checkout 切换到新项目
        app.ui.update_step("Checking out the new project");
        svn_checkout(&[&trunk_url, "."])?;
        // checkout 成功了，有 ,svn 目录了，链接根目录的 .gitignore 文件
        svn_propset(&["svn:externals", "^/.gitignore .gitignore", "."])?;
        // 先检查本地有没有.gitignore 文件，有的话先改名
        if std::path::Path::new(".gitignore").exists() {
            fs::rename(".gitignore", ".conflict.gitignore")?;
            app.ui.warn(&format!("{} {}", "Local .gitignore file exists, renamed to", ".conflict.gitignore".yellow().bold()));
        }
        svn_update(&["."])?;
        app.ui.success(&format!("Checked out to the new project {}", project_name.yellow().bold()));
    }
    else {
        let switch_to_new = app.ui.selector_yes_or_no("Switch to the new project now?")?;
        if switch_to_new {
...
/// 切换当前工作区到另一个项目
pub fn handle_switch(app: &App, project_name: Option<&str>, branch: Option<&str>) -> AppResult<()> {
    app.ui.update_step("Parsing target project");
    let target_project = project_name.unwrap_or_else(|| app.svn_ctx.get_current_project_name());
    let target_subpath = branch.map(|b| format!("branches/{}", b)).unwrap_or_else(|| "trunk".to_string());
